...
/**
  * @brief  基本定时器TIM5作为计数模式初始化
  * @param  prescaler 				时钟预分频
  *			period					定时器重装载寄存器的值
  *			定时器定时频率：系统时钟频率/预分频/重装载值
  * @retval None
  */
void TIM5_CounterMode(u16 prescaler, u16 period){
	//使能定时器时钟
...
/**
  ...
  * @retval None
  */
static void TIM4_GPIOInit(void){
	GPIO_InitTypeDef GPIO_InitStructure;

	RCC_AHB1PeriphClockCmd(TIM4_CH1_GPIO_CLK | TIM4_CH2_GPIO_CLK, ENABLE);
