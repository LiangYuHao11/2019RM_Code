...
void Ramp_MecanumCalculate(float Vx, float Vy, float VOmega, int16_t *Speed)
{
 float tempSpeed[4];
	...
	//速度限制
	VAL_LIMIT(Vx, -1600, 1600);  
	VAL_LIMIT(Vy, -1600, 1600);  
	VAL_LIMIT(VOmega, -1600, 1600);  
...
//	Chassis_powerLimit(M3508s,3440.0f);
	
//  Chassis.allPID = (abs(M3508s[0].outCurrent) + abs(M3508s[1].outCurrent) +abs(M3508s[2].outCurrent) +abs(M3508s[3].outCurrent))*1.22f;

//	if( MOTOR0 == 1)
//	{
//   M3508s[0].outCurrent = 0;
//	}
//	
//	if( MOTOR1 == 1)
//	{
//	  M3508s[1].outCurrent = 0;
//	}
//	if( MOTOR2 == 1)
//	{ M3508s[2].outCurrent = 0;}
//	if( MOTOR3 == 1)
//	{M3508s[3].outCurrent = 0;}
